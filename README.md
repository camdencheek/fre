...
The primary difference is `topd` does not actually support jumping. 
...
`topd` is primarily designed to interface with `fzf`. For general usage, 
...
Then, `topd` can be used as a source for `fzf`. I personally use the `fzf`-provided 
control-T bindings, modified to use `topd` as input. Some examples are below.
...
To preferentially use results from topd, but fall back to other results, we can use 
...
used. The following options first use `topd` results, then use all the subdirectories 
